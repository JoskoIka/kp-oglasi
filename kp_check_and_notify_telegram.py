...
def parse_ads(html):
    ...
    for sec in soup.select('section[class*="AdItem_adOuterHolder"]'):
        ...
        ads.append({
            "link": urljoin("https://www.kupujemprodajem.com", a["href"]),
...
def process_search(cfg):
    ...
    old_links = set(json.load(open(state_file))) if os.path.exists(state_file) else set()
    ...
    json.dump([a["link"] for a in ads], open(state_file,"w"), ensure_ascii=False, indent=2)
    return len(new_ads)
